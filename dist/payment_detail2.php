...
$cus_bill_id = $_REQUEST["cus_bill_id"];
$name = $_REQUEST["name"];
...
// echo $namme=$branchTable1['name'];

if (isset($_POST['submit'])) {
    $payment_date = $_POST["payment_date"];
	$amount_enter = $_POST["amount_enter"];
	$mode_payment = $_POST["mode_payment"];
	$balance = $_POST["balance"];
	$already_paid1 = $_POST["already_paid1"];
	$name = $_POST["party11"];

	if ($mode_payment == "cash" && $payment_date == "") {
		$sql1 = "select id,from_cx_id,name,from_contact,to_address,mode_payment,createdAt11,SUM(total1) as total1,SUM(paymnent) as paymnent1,SUM(balance) as balance1,payment_date from party where name='$name' GROUP BY name";
		...
		if ($balance >= $amount_enter) {
			$total_balance = $balance - $amount_enter;
			$total_payment = $payment + $amount_enter;
			
			$sql2 = "update party set name='$name',balance=balance-'$amount_enter',paymnent=paymnent+'$amount_enter',amount_enter='$amount_enter',
		mode_payment='$mode_payment',createddate1='$createdat',payment_date='$date' where name='$name' and id='$id'";
            			if ($conn->query($sql2)) {
        //   $result1 = $conn->query($sql2);
        // 		$branchTable1 = $result1->FETCH_ASSOC(); {
        // 		echo $createdat = $branchTable1['createddate1'];
        
        	            $sql217 = "SELECT * FROM party where name='$name' and id='$id'";
						$result27 = $conn->query($sql217);
						$branchTable = $result27->fetch_assoc();
						$pymtdate = $branchTable['payment_date'];

				$sql1 = "SELECT COUNT(id) as base_count FROM base where from_name='$name'";
				...
				if ($no > 0) {
					for ($i = 0; $i < $no; $i++) {
						...
						$sql217 = "SELECT * FROM base WHERE from_name='$name' AND amount_paid1=0 AND bill_id!='' ORDER BY id ASC LIMIT 1";
						$result27 = $conn->query($sql217);
						$branchTable = $result27->fetch_assoc();
						$amount_paid1 = $branchTable['amount_paid1'];
						$id = $branchTable['id'];
						$bill_id = $branchTable['bill_id'];
						
						if ($amount_paid1 == 0) {

				           	// 	echo $sql22 = "update base set amount_paid1='$amount_enter',already_paid1='$amount_enter',pymt_date='$pymtdate' where id='$id'";
						$sql22 = "update base set amount_paid1='$amount_enter',already_paid1='$amount_enter',pymt_date='$pymtdate' where id='$id' AND bill_id='$bill_id'";
							...
							if ($conn->query($sql22)) {
								...
								if ($amount_enter) {
									$sql27 = "UPDATE add_branch SET branch_balance = (branch_balance+'$amount_enter') WHERE branch_name= '$userBranch'";
									if ($conn->query($sql27)) {
										...
										$sql37 = "INSERT INTO transaction_history (branch_id,user_id,username,party_name,category,Amount,credit,debit,balance,branch_name,mode_payment,description)  
                                VALUES ('$branch_id','$userId','$username','$name','Payments','$amount_enter','$amount_enter',0,'$branch_balance','$userBranch','$mode_payment','Party Paid')";
										$conn->query($sql37);

										$sql2 = "insert into partyexcel (name,from_cx_id,from_contact,total,balance,paymnent,amount_enter,mode_payment,createddate1,username,branch) 
values ('$name','$from_cx_id','$from_contact','$total','$total_balance','$total_payment','$amount_enter','$mode_payment','$createdat','$username','$branch')";
										$conn->query($sql2);
									}
									break;
								}
			...
			echo "<script type='text/javascript'>
                     $(document).ready(function() {
                      Swal.fire({
                          text: 'Enter Amount Greater than Balance',
                          icon: 'danger',
                          confirmButtonColor: '#1BC5BD',
                          button: 'Dashboard!',
                        })
			...
			// alert("Enter Amount Greater than Balance");
		}}
	else if ($mode_payment != "cash" && $payment_date == "") {
		$sql1 = "select id,from_cx_id,name,from_contact,to_address,mode_payment,createdAt11,SUM(total1) as total1,SUM(paymnent) as paymnent1,SUM(balance) as balance1,payment_date from party where name='$name' GROUP BY name";
		$result1 = $conn->query($sql1);
		while ($branchTable = $result1->FETCH_ASSOC()) {
			$balance =  $branchTable['balance1'];
						...
						if ($amount_paid1 == 0) {
							...
							if ($conn->query($sql22)) {
								...
								if ($amount_enter) {
									...
									$sql37 = "INSERT INTO transaction_history (branch_id,user_id,username,party_name,category,Amount,credit,debit,balance,branch_name,mode_payment,description)  
                                VALUES ('$branch_id','$userId','$username','$name','Payments','$amount_enter',0,0,'$branch_balance','$userBranch','$mode_payment','Party Paid')";
										$conn->query($sql37);
...
values ('$name','$from_cx_id','$from_contact','$total','$total_balance','$total_payment','$amount_enter','$mode_payment','$createdat','$username','$branch')";
		...
		if ($balance >= $amount_enter) {
			...
			$sql2 = "update party set name='$name',balance=balance-'$amount_enter',paymnent=paymnent+'$amount_enter',amount_enter='$amount_enter',
		mode_payment='$mode_payment',createddate1='$createdat',payment_date='$payment_date' where name='$name' and id='$id'";
			if ($conn->query($sql2)) {
...
<body id="kt_body" class="header-mobile-fixed subheader-enabled aside-enabled aside-fixed aside-secondary-enabled page-loading">
						...
						?>
								...
								<div class="col-lg-12">
									<form class="login-form" id="register_form" method="post" enctype="multipart/form-data">
										<!--<form class="login-form" id="kt_form_11" method="post" enctype="multipart/form-data">-->
										<!--begin::Card-->
										<div class="row">
											<div class="col-lg-6">
												<div class="card card-custom  example-compact">
													<div class="card-body">
														<div class="form-group row">
															...
															<select class="form-control" name="party11" id="party11">
																...
																?>
																	<option value="<?= $branchTable1['name'] ?> "><?= $branchTable1['name']  ?></option>
																<?php } ?>
															</select>
														</div>
														<input type="hidden" class="form-control" name="namme" id="namme" value="<?= $branchTable1['name'] ?>">
														<input type="hidden" class="form-control" name="namme" id="namme" value="<?= $branchTable1['name'] ?>">

														<!--<div class="form-group row" id="v">-->
														<!--    <label class="form-label">Party Bill </label>-->
														<!--    <select class="form-control" name="partybill11" id="partybill11">-->
														<!--        <option selected disabled>Select Bill </option>-->
														<!--    </select>-->
														<!--</div>-->
													</div>
												</div>
												<input type="hidden" name="cus_bill_id" id="cus_bill_id">
											...
											<div class="col-lg-6">
												<div class="card card-custom example-compact">
													<div class="card-body">
														...
														<!--	<input type="text" class="form-control" name="totalbill" id="totalbill" readonly>-->
														<!--</div>-->
														<div class="form-group row">
															<label class="form-label">Amount Paid</label>
															<input type="text" class="form-control" name="amountbill" id="amountbill" readonly>
														</div>
														<div class="form-group row">
															<label class="form-label">Balance</label>
															<input type="text" class="form-control" name="balancebill" id="balancebill" readonly>
														</div>
														<div class="form-group row">
														    <label class="form-label">from</label>
															<input type="text" class="form-control" name="anountenterdetail" id="anountenterdetail" >
														</div>
													</div>
												</div>
									...
									<form class="login-form" id="register_form" method="post" enctype="multipart/form-data">
										...
										<div class="row">
											<div class="col-lg-6">
													...
													<div class="card-body">
														<div class="form-group row">
															...
															<select class="form-control" name="party11" id="party11">
																<option value="">Select</option>
																<?php
																$sql1 = "SELECT * FROM party where branch1 = '$branch' GROUP BY name ";
																$result1 = $conn->query($sql1);
																while ($branchTable1 = $result1->fetch_assoc()) {
																?>
																	<option value="<?= $branchTable1['name'] ?> "><?= $branchTable1['name']  ?></option>
														...
														<!--<div class="form-group row" id="v">-->
														<!--    <label class="form-label">Party Bill </label>-->
														<!--    <select class="form-control" name="partybill111" id="partybill111">-->
														<!--        <option selected disabled>Select Bill </option>-->
														<!--    </select>-->
														<!--</div>-->
													</div>
												...
												<div class="card card-custom  example-compact" id="anountenterdetail">
													<div class="card-body">
														<div class="form-group row">
															<label class="form-label">Amount</label>
															<input type="text" class="form-control" name="amount_enter" id="amount_enter" placeholder="Enter your amount" required />
														</div>
														<div class="form-group row">
															<form>
													...
													<div class="card-body">
														<div class="form-group row">
															...
															<select class="form-control" name="party11" id="party11">
																<option value="">Select</option>
																<?php
																$sql1 = "SELECT * FROM party where branch1 = '$branch' && username1 = '$username' GROUP BY name";
																$result1 = $conn->query($sql1);
																while ($branchTable1 = $result1->fetch_assoc()) {
																?>
																	<option value="<?= $branchTable1['name'] ?> "><?= $branchTable1['name']  ?></option>
														...
														<!--<div class="form-group row" id="v">-->
														<!--    <label class="form-label">Party Bill </label>-->
														<!--    <select class="form-control" name="partybill112" id="partybill112">-->
														<!--        <option selected disabled>Select Bill </option>-->
														<!--    </select>-->
														<!--</div>-->
													</div>
	...
	<script>
		$('#party11').on('change', function() {
			var name = $('#party11').find(":selected").val();
// 			alert(name);
			$.ajax({
				...
				dataType: "json",
				data: {
					"action": "payment_from_detail10",
					"name": name
				},
				success: function(result_job) {
					// $('#party11').html('<option selected disabled>Select Bill </option>');
					$('#anountenterdetail').html('');
					$('#amountbill').html('');
					$('#balancebill').html('');
					$('#cus_bill_id').html('');
					var length = result_job.data.length;
					for (var i = 0; i < length; i++) {
						  //alert(result_job.data[i].total1);
						  //alert(result_job.data[i].balance);

						$('#anountenterdetail').html('<div class="card-body"><div class="form-group row"><label class="form-label">Date</label><input type="datetime-local" value="<?php echo $date ?>" class="form-control" name="payment_date" id="payment_date" placeholder="Enter your LR No" required /></div><div class="form-group row"><label class="form-label">Amount</label><input type="text" class="form-control" name="amount_enter" id="amount_enter" placeholder="Enter your amount" required /></div><div class="form-group row"> <form><label class="form-label">Mode of Payment</label><select class="form-control" name="mode_payment" id="mode_payment" required><option value="">Select</option><option value="cash">Cash</option><option value="cheque">Cheque</option><option value="upi">UPI</option><option value="neft">NEFT</option><option value="rtgs">RTGS</option></select></form></div><button type="submit" class="btn btn-primary font-weight-bold" name="submit" id="submit">Submit</button></div>');
						// $('#totalbill').append( "<option>" + result_job.data[i].total1 + "</option>");
						$('#amountbill').val(result_job.data[i].amount_enter);
						$('#balancebill').val(result_job.data[i].balance);
					}
				}
			});
		});
		...
		//     let x = this.value;
		//     let arr = x.split("_");
		//     $('#cus_bill_id').val(arr[0]);
		//     $('#totalbill').val(arr[1]);
		//     $('#amountbill').val(arr[2]);
		//     $('#balancebill').val(arr[3]);
		//     console.log()
		// });
	</script>
	<script>
		$('#party11').on('change', function() {
			...
			$.ajax({
				...
				dataType: "json",
				data: {
					"action": "payment_from_detail11",
					"name": name,
				},
				success: function(result_job) {
					// alert(result_job.data[0].bill_id);
					$('#totalbill').html('');
					$('#amountbill').html('');
					$('#balancebill').html('');
					$('#cus_bill_id').html('');
					var length = result_job.data.length;
					for (var i = 0; i < length; i++) {
						// alert(result_job.data[i].bill_id);

						$('#totalbill').val(result_job.data[i].total1);
						// $('#totalbill').append( "<option>" + result_job.data[i].total1 + "</option>");
						$('#amountbill').val(result_job.data[i].amount_enter);
						$('#balancebill').val(result_job.data[i].balance);
					}
		...
		$('#party11').on('change', function() {
			...
			$.ajax({
				...
				success: function(result_job) {
					...
					for (var i = 0; i < length; i++) {
						// alert(result_job.data[i].total1);
						$('#totalbill'1).val(result_job.data[i].total1);
						// $('#totalbill').append( "<option>" + result_job.data[i].total1 + "</option>");
						$('#amountbill').val(result_job.data[i].amount_enter);
						$('#balancebill').val(result_job.data[i].balance);
						...
						// alert(result_job.data[i].bill_id);
						// $('#v').append('<input type="text" name="cus_bill_id" id="cus_bill_id" value="' + result_job.data[i].bill_id + '">');

						// $('#partybill11').append("<option value="+ result_job.data[i].bill_id">" + result_job.data[i].bill_id+"</option>");
						// if (result_job.data[i].balance != 0) {
						//     $('#partybill112').append("<option value=" + result_job.data[i].bill_id + "_" + result_job.data[i].total + "_" + result_job.data[i].paymnent + "_" + result_job.data[i].balance + "_" + result_job.data[i].from_date + "_" + result_job.data[i].to_date + ">" + result_job.data[i].bill_id + " - " + result_job.data[i].from_date + " to " + result_job.data[i].to_date + "</option>");
						// }
						// $from_name = $_GET[result_job.data[i].bill_id];
						// $('#totalbill').append("<option>" + result_job.data[i].total + "</option>");
					}
				}
			});
		});
