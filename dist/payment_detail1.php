...
if (isset($_POST['submit'])) {
    ...
    $sql1 = "select * from party where bill_id='$cus_bill_id'";
    $result1 = mysqli_query($conn, $sql1);
    $branchTable = $result1->fetch_assoc();
    $balance =  $branchTable['balance'];
    if ($balance >= $amount_enter) {

        $sql2 = "update party set name='$name',balance=balance-'$amount_enter',paymnent=paymnent+'$amount_enter',amount_enter='$amount_enter',mode_payment='$mode_payment',createddate1='$createdat' where bill_id='$cus_bill_id'";
        if ($conn->query($sql2)) {

            $sql1 = "SELECT COUNT(id) as base_count FROM base where bill_id='$cus_bill_id'";
            $result1 = mysqli_query($conn, $sql1);
            $rec1 = mysqli_fetch_assoc($result1);
            $no =  $rec1['base_count'];
            if ($no > 0) {
                for ($i = 0; $i < $no; $i++) {
                    $sql217 = "SELECT * FROM base WHERE bill_id='$cus_bill_id' AND amount_paid1=0 ORDER BY id ASC LIMIT 1";
                    ...
                    if ($amount_paid1 == 0) {
                        ...
                        $sql22 = "update base set amount_paid1='$amount_enter',already_paid1='$amount_enter' where id='$id'";
                        if ($conn->query($sql22)) {
                            ...
                            if ($amount_enter) {
                                ...
                                if ($conn->query($sql27)) {
                                    ...
                                    $sql37 = "INSERT INTO transaction_history (branch_id,user_id,from_name,category,Amount,credit,debit,balance,branch_name)  
                                VALUES ('$branch_id','$userId','$name','Payments','$amount_enter','$amount_enter',0,'$branch_balance','$userBranch')";
                                    $conn->query($sql37);
                                }
                                break;
                            }
                        ...
                        ?>
                            ...
                            <div class="col-lg-12">
								...
								<select class="form-control" name="mode_payment" id="mode_payment" required>
									<option selected disabled>Select</option><option value="cash">Cash</option>
									<option value="cheque">Cheque</option><option value="upi">UPI</option>
									<option value="neft">NEFT</option><option value="rtgs">RTGS</option>
								</select>
							</div>
                                                    <button type="submit" class="btn btn-primary font-weight-bold" name="submit" id="submit">Submit</button>
                                                </div>
                                            </div>
    ...
    <script>
        $('#party11').on('change', function() {
            ...
            $.ajax({
                ...
                dataType: "json",
                data: {
                    "action": "payment_from_detail",
                    "name": name
                },
                success: function(result_job) {
                    // alert(result_job.data[0].bill_id);
                    $('#partybill11').html('<option selected disabled>Select Bill </option>');
                    $('#totalbill').html('');
                    $('#amountbill').html('');
                    $('#balancebill').html('');
                    $('#cus_bill_id').html('');
                    ...
                    for (var i = 0; i < length; i++) {
                        ...
                        if (result_job.data[i].balance != 0) {
                            $('#partybill11').append("<option value=" + result_job.data[i].bill_id + "_" + result_job.data[i].total1 + "_" + result_job.data[i].paymnent + "_" + result_job.data[i].balance + "_" + result_job.data[i].from_date + "_" + result_job.data[i].to_date + ">" + result_job.data[i].bill_id + " - " + result_job.data[i].from_date + " to " + result_job.data[i].to_date + "</option>");
                        }
        ...
        $("#partybill11").on('change', function() {
            let x = this.value;
            let arr = x.split("_");
    ...
    <script>
        $('#party11').on('change', function() {
            ...
            $.ajax({
                ...
                dataType: "json",
                data: {
                    "action": "payment_from_detail1",
                    "name": name,
                },
                success: function(result_job) {
                    // alert(result_job.data[0].bill_id);
                    $('#partybill111').html('<option selected disabled>Select Bill </option>');
                    $('#totalbill').html('');
                    $('#amountbill').html('');
                    $('#balancebill').html('');
                    $('#cus_bill_id').html('');
                    ...
                    for (var i = 0; i < length; i++) {
                        ...
                        // $('#partybill11').append("<option value="+ result_job.data[i].bill_id">" + result_job.data[i].bill_id+"</option>");
                        if (result_job.data[i].balance != 0) {
                            $('#partybill111').append("<option value=" + result_job.data[i].bill_id + "_" + result_job.data[i].total1 + "_" + result_job.data[i].paymnent + "_" + result_job.data[i].balance + "_" + result_job.data[i].from_date + "_" + result_job.data[i].to_date + ">" + result_job.data[i].bill_id + " - " + result_job.data[i].from_date + " to " + result_job.data[i].to_date + "</option>");
                        }
                        // $from_name = $_GET[result_job.data[i].bill_id];
                        // $('#totalbill').append("<option>" + result_job.data[i].total + "</option>");
                    }
        ...
        $("#partybill111").on('change', function() {
            let x = this.value;
            let arr = x.split("_");
            $('#cus_bill_id').val(arr[0]);
            $('#totalbill').val(arr[1]);
     ...
     <script>
        $('#party11').on('change', function() {
            ...
            $.ajax({
                ...
                dataType: "json",
                data: {
                    "action": "payment_from_detail2",
                    "name": name,
                },
                success: function(result_job) {
                    // alert(result_job.data[0].bill_id);
                    $('#partybill112').html('<option selected disabled>Select Bill </option>');
                    $('#totalbill').html('');
                    $('#amountbill').html('');
                    $('#balancebill').html('');
                    $('#cus_bill_id').html('');
                    ...
                    for (var i = 0; i < length; i++) {
                        ...
                        // $('#partybill11').append("<option value="+ result_job.data[i].bill_id">" + result_job.data[i].bill_id+"</option>");
                        if (result_job.data[i].balance != 0) {
                            $('#partybill112').append("<option value=" + result_job.data[i].bill_id + "_" + result_job.data[i].total1 + "_" + result_job.data[i].paymnent + "_" + result_job.data[i].balance + "_" + result_job.data[i].from_date + "_" + result_job.data[i].to_date + ">" + result_job.data[i].bill_id + " - " + result_job.data[i].from_date + " to " + result_job.data[i].to_date + "</option>");
                        }
                        // $from_name = $_GET[result_job.data[i].bill_id];
                        // $('#totalbill').append("<option>" + result_job.data[i].total + "</option>");
                    }
        ...
        $("#partybill112").on('change', function() {
            let x = this.value;
            let arr = x.split("_");
            $('#cus_bill_id').val(arr[0]);
            $('#totalbill').val(arr[1]);
